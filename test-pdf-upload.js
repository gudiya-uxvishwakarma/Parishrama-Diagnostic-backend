...
async function testPdfUpload() {
  try {
    ...
    const appointmentsResponse = await fetch('https://parishrama-diagnostic-backend-1.onrender.com/api/appointments');
...
%%EOF`;
    ...
    const uploadResponse = await fetch(
      `https://parishrama-diagnostic-backend-1.onrender.com/api/appointments/${testAppointment._id}/upload-report`,
      {
        method: 'POST',
        body: formData,
      }
    ...
    if (uploadResult.success) {
      ...
      const downloadUrl = `https://parishrama-diagnostic-backend-1.onrender.com/api/appointments/${testAppointment._id}/download-report`;
